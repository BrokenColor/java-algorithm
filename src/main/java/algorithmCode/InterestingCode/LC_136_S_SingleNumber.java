...
public class LC_136_S_SingleNumber {
    ...
    public int singleNumber1(int[] nums) {
        int a = 0;
        /**
         * 利用异或运算的性质找到数组中只出现一次的数字：
         * 1. 交换律：a ^ b ^ c <=> a ^ c ^ b
         * 2. 任何数于0异或为任何数 0 ^ n => n
         * 3. 相同的数异或为0: n ^ n => 0
         */
        for (int i = 0; i < nums.length; i++) {
            a ^= nums[i]; // 遍历数组，并对目前的结果和当前数字进行异或操作
        }
        return a; // 返回最终异或结果，即数组中只出现一次的数字
    }


    /**
     * 找出数组中只出现一次的数字
     * @param nums 整型数组，包含重复出现的数字和一个只出现一次的数字
     * @return 返回只出现一次的数字
     */
    public int singleNumber(int[] nums) {
        ...
        while (true) {
            // 如果列表中相邻的元素相同，则移除这两个元素
            if (arr.size() > 1 && arr.get(0).equals(arr.get(1))) {
                arr.remove(0);
                arr.remove(0);
            } else {
                break;
        ...
        // 打印系统信息和剩余元素
        System.out.println("sys:" + arr.get(0));
